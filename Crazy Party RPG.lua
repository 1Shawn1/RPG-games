...
local function KillEnemys()
	while wait(game:GetService("Stats").PerformanceStats.Ping:GetValue() / 750 * 2)  do
		if getgenv().KillEnemys then
			...
			if GetBestSword().Name == "The Universal Pitchfork" and game:GetService("Workspace").Mobs:FindFirstChild("elden lober") then
				for i = 1, 50 do
					DamageRemote:FireServer(game:GetService("Workspace").Mobs:FindFirstChild("elden lober"):FindFirstChildOfClass("Part"), game:GetService("Workspace").Mobs["elden lober"].Enemy, LocalPlayer.Character:FindFirstChildOfClass("Tool"))
				end
			end
		end
	end
